...
from terminal import InputBuffer, Paste, PasteRange, display_text
...
def test_left_into_paste_end():
    ...
    ti.handle_key("left")  # from 7 (paste end) → 1 (paste start)
    assert ti.cursor == 1
...
def test_word_left_stops_at_paste_typed_boundary():
    ...
    assert ti.cursor == 9
    ti.handle_key("word-left")  # should stop at start of "dsa" (pos 6)
    assert ti.cursor == 6
    ti.handle_key("word-left")  # should jump over paste to 0
    assert ti.cursor == 0
...
def test_word_right_stops_at_typed_paste_boundary():
    ...
    ti.cursor = 0
    ti.handle_key("word-right")  # should stop at end of "dsa" (pos 3)
    assert ti.cursor == 3
    ti.handle_key("word-right")  # should jump over paste to 9
    assert ti.cursor == 9
...
def test_word_nav_two_pastes_then_typed():
    ...
    ti = InputBuffer()
    ti.handle_key(Paste("aaaa"))
    ti.handle_key(Paste("bbbb"))
    for c in "dsa":
        ti.handle_key(c)
    # value = "aaaabbbbdsa", paste1=[0,4), paste2=[4,8), typed="dsa"
    assert ti.cursor == 11
    ti.handle_key("word-left")  # stop at start of "dsa" (pos 8)
    assert ti.cursor == 8
    ti.handle_key("word-left")  # skip paste2 → pos 4
    assert ti.cursor == 4
    ti.handle_key("word-left")  # skip paste1 → pos 0
    assert ti.cursor == 0
    # Now go right
    ti.handle_key("word-right")  # skip paste1 → pos 4
    assert ti.cursor == 4
    ti.handle_key("word-right")  # skip paste2 → pos 8
    assert ti.cursor == 8
    ti.handle_key("word-right")  # end of "dsa" → pos 11
    assert ti.cursor == 11
