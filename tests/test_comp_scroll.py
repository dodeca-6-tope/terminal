...
def test_clamps_offset_over_max():
    s = _state(10)
    result = clean(scroll(text("a"), text("b"), text("c"), state=s).render(80, 2))
    assert result == ["b", "c"]
    assert s.offset == 1
...
def test_offset_clamps_when_content_fits():
    s = _state(5)
    result = clean(scroll(text("a"), text("b"), state=s).render(80, 5))
    assert result == ["a", "b", "", "", ""]
...
def test_multiline_child_partial_clip():
    ...
    assert clean(scroll(child, text("after"), state=s).render(80, 2)) == [
        "x",
        "y",
    ]
...
def test_multiline_child_at_offset():
    ...
    assert clean(scroll(child1, child2, child3, state=s).render(80, 2)) == [
        "c",
        "d",
    ]
...
def test_flex_basis_uses_max():
    s = _state()
    sc = scroll(text("hi"), text("hello"), state=s)
...
def test_dimensions_update_on_rerender():
    s = _state()
    sc1 = scroll(text("a"), text("b"), state=s)
    sc1.render(80, 5)
    assert s.total == 2
    assert s.height == 5
    # Simulate rerender with different children
    sc2 = scroll(text("a"), text("b"), text("c"), text("d"), state=s)
    sc2.render(80, 3)
    assert s.total == 4
    assert s.height == 3
...
def test_state_persists_offset_across_renders():
    ...
    result = clean(scroll(*[text(str(i)) for i in range(10)], state=s).render(80, 3))
    assert result == ["5", "6", "7"]
...
def test_page_down_then_render():
    ...
    s.page_down()
    result = clean(scroll(*[text(str(i)) for i in range(20)], state=s).render(80, 5))
...
def test_follow_tracks_growing_content_end_to_end():
    ...
    s = ScrollState(follow=True)
    items = [text(str(i)) for i in range(5)]
    result = clean(scroll(*items, state=s).render(80, 3))
    assert result == ["2", "3", "4"]
